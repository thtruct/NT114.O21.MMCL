...
export type CognitoContextType = CanRemove & {
  user: AuthUserType;
  method: string;
  loading: boolean;
  authenticated: boolean;
  unauthenticated: boolean;
  login: (email: string, password: string) => Promise<void>;
  loginByAuthorizeCode: (code: string, state: string) => Promise<void>;
  ...
  confirmRegister: (email: string, code: string) => Promise<void>;
  resendCodeRegister: (email: string) => Promise<void>;
  generatePKCE: () => Promise<{ state: string; codeVerifier: string; codeChallenge: string }>;
  generateAuthorizeUri: (idPName: ESocialProvider) => Promise<string>;
